...
def evaluate(hparams_file, run_opts, overrides):
    ...
    for k in test_datasets.keys():  # keys are test_clean, test_other etc
        ...
        target_brain.hparams.adv_wer_file = os.path.join(
            hparams["output_folder"], "adv_wer_{}.txt".format(k)
        )
        target_brain.hparams.my_log_file = os.path.join(hparams["output_folder"], "log_wer.txt")
        f = open(target_brain.hparams.my_log_file, "w")
        f.write("Hello world!")
        f.close()

        print(type(test_datasets[k]))
