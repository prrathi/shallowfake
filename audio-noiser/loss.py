...
def get_loss_single_segment(
    ...
    **options,
    ...
    if audio.ndim == 1:
        audio = audio.to(model.device).to(torch.float32)
        mel = log_mel_spectrogram(audio).to(torch.float32)
    else:
        mel = audio.to(model.device).to(torch.float32) 
    ...
    return dict(loss=loss, logits=logits, text=result.text)
